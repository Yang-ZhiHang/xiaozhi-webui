# https://github.com/huangjunsen0406/py-xiaozhi/blob/main/src/utils/opus_loader.py
...
# 目录结构定义 - 根据实际目录结构定义路径
DIR_STRUCTURE = {
    WINDOWS: {"arch": "x86_64", "path": "libs/win/x86_64"},
    MACOS: {
...
def get_search_paths(system: str, arch_name: str):
    """
    获取库文件搜索路径列表（使用统一的资源查找器）
    """
    from ..constants import WORK_DIR
    from .finder import find_libs_dir

    lib_name: str = LIB_INFO[system]["name"]
    ...
    if system_dir:
        specific_libs_dir = find_libs_dir(system_dir, arch_name)
    ...
    # 然后查找特定平台的libs目录
    if system_dir:
        platform_libs_dir = find_libs_dir(system_dir)
    ...
    for dir_path, filename in search_paths:
        ...
        logger.debug(f"搜索路径: {full_path} (存在: {os.path.exists(full_path)})")
    return search_paths
